    ...
    return (
      <DashboardLayout>
        <div className="container mx-auto max-w-3xl px-4 py-10">
          <p className="text-gray-700">You need administrator access to view the SEO index tracker.</p>
        </div>
      </DashboardLayout>
    );
  }
        ...
        <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600">
              ...
              {lastRunAt && (
                <span className="ml-2 text-gray-500">Last scan: {lastRunAt.toLocaleString()}</span>
              )}
            </p>
          </div>
          ...
          <CardContent className="overflow-x-auto p-0">
            ...
            {results.length > 0 && (
              <table className="w-full min-w-[760px] text-sm">
                ...
                <tbody>
                  {filtered.map((r) => (
                    <tr key={r.url} className="border-t border-gray-100 align-top">
                      <td className="px-4 py-3 font-mono text-xs text-gray-800">
                        {r.url.replace(SITE_ORIGIN, "") || "/"}
                      </td>
                      <td className="px-4 py-3">{verdictBadge(r.verdict, !!r.error)}</td>
                      <td className="px-4 py-3 text-gray-700">{r.coverageState ?? "—"}</td>
