...
const FALLBACK_PENDING_JOBS = 24;
const FALLBACK_HIGH_PRIORITY_JOBS = 3;
const FALLBACK_ACTIVE_TECHS = 12;
const FALLBACK_TOTAL_TECHS = 15;
...
const Dashboard = () => {
  ...
  const displayedPendingJobs = isLoading
    ...
    : hasLiveWorkOrders
      ? pendingJobs.length
      : FALLBACK_PENDING_JOBS;
  const displayedHighPriority = hasLiveWorkOrders
    ? highPriorityCount
    : FALLBACK_HIGH_PRIORITY_JOBS;
  const displayedActiveTechs = isLoading
    ? "—"
    : activeTechs || FALLBACK_ACTIVE_TECHS;
  const displayedTotalTechs = Math.max(activeTechs, FALLBACK_TOTAL_TECHS);
