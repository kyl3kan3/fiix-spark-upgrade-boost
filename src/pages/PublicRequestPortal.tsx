    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
        ...
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-600 mb-6">This request portal link is invalid or has been removed.</p>
          ...
          <>
            <div className="grid sm:grid-cols-2 gap-3 mb-8">
              <button
                ...
                className={`p-5 rounded-lg border-2 text-left transition-all ${type === "standard" ? "border-maintenease-600 bg-maintenease-50" : "border-gray-200 bg-white hover:border-gray-300"}`}
              >
                <div className="flex items-center gap-2 mb-1">
                  <Wrench className="h-5 w-5 text-maintenease-600" />
                  <span className="font-semibold">Submit a request</span>
                </div>
                <p className="text-sm text-gray-600">Routine maintenance, repairs, or improvements.</p>
              </button>
              <button
                type="button"
                onClick={() => setType("urgent")}
                className={`p-5 rounded-lg border-2 text-left transition-all ${type === "urgent" ? "border-red-600 bg-red-50" : "border-gray-200 bg-white hover:border-red-300"}`}
              >
                <div className="flex items-center gap-2 mb-1">
                  <AlertTriangle className="h-5 w-5 text-red-600" />
                  <span className="font-semibold text-red-700">Urgent — needs fixing now</span>
                </div>
                <p className="text-sm text-gray-600">Safety risk, leak, no heat/AC, power, equipment down.</p>
              </button>
            </div>

            <form onSubmit={onSubmit} className="bg-white rounded-xl border border-gray-200 p-6 sm:p-8 space-y-5">
              ...
              <p className="text-xs text-gray-500 text-center">
                Powered by <Link to="/" className="underline hover:text-gray-700">MaintenEase</Link>
              </p>
            </form>
          </>
        )}
