...
export function useAuthForm({ isSignUp, onError }: UseAuthFormProps) {
 ...
 const { handleSignIn, handleSignUp, isLoading } = useAuthSubmission({ onError });
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent, turnstileToken?: string) => {
 e.preventDefault();
 
 if (isSignUp) {
      await handleSignUp(formData.email, formData.password, formData.name, formData.companyName, turnstileToken);
 } else {
