...
/**
 * Bulk-creates the freezer assets for every group and attaches them to a
 * new or existing inspection checklist, staggering prompts 15 min apart.
 */
    ...
    mutationFn: async () => {
      ...
      for (const g of groups) {
        ...
        if (g.mode === "new") {
          ...
          const offsets: Record<string, number> = { ...(target?.asset_offsets || {}) };
