...
export async function parseDOCX(file: File, expectedCount?: number): Promise<any[]> {
  ...
  // If the text is short, treat it as a single vendor with info extraction
  if (text.length < 200) {
  ...
  if (expectedCount) {
    // If expecting 1 vendor, treat as single vendor with info extraction
    if (expectedCount === 1) {
      return [extractVendorInfo(text)];
    }
  ...
  // Otherwise, treat the entire document as one vendor entry with info extraction
  return [extractVendorInfo(text)];
}
