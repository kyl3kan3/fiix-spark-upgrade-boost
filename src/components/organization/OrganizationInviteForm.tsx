...
export const OrganizationInviteForm: React.FC<OrganizationInviteFormProps> = ({ organizationId }) => {
 ...
 <div className="mt-6">
 <h3 className="font-semibold mb-2 text-foreground">Pending Invitations</h3>
 {pendingInvites.length === 0 && (
 <div className="text-sm text-muted-foreground">No pending invites.</div>
 )}
 <ul className="divide-y rounded border bg-card mt-2">
 {pendingInvites.map(invite => (
 <li key={invite.id} className="flex flex-col md:flex-row md:items-center justify-between py-2 px-3">
 <div>
 <span className="font-medium">{invite.email}</span>
 <span className="ml-2 px-2 py-0.5 text-xs rounded bg-muted text-foreground">{invite.role}</span>
 </div>
 {invite.token && (
