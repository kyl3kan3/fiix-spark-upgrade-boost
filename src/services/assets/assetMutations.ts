...
export async function deleteAsset(assetId: string) {
  ...
  // Attempt deletion using service role key if available or regular deletion
  ...
  if (count === 0) {
    ...
    if (!verifyGone) {
      console.log('✅ deleteAsset service - Asset successfully deleted (count was inaccurate)');
      return;
    }
